...
# Global variables for target development
# The target repository is https://github.com/epiusegs/altrixone_framework
# The target development branch should be prexifed with the version and then the story id / task id (ticket id) e.g. v15.0_ALTRX-123
# Create your dev or qa branch prior to running the script / codespace
...
def main():
    global target_project, target_repository, target_branch
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="frappe repo to use, default: https://github.com/frappe/frappe.git",  # noqa: E501
...
def create_site_in_bench(args):
    ...
    if "mariadb" == args.db_type:
        ...
        new_site_cmd = [
            ...
            f"--db-name=development.localhost.db",
            f"--no-mariadb-socket",
    ...
    # Uninstall apps in the required order
    apps_to_uninstall = ["payments", "erpnext", "hrms"]
    for app in apps_to_uninstall:
        cprint(f"Uninstalling app {app} ...", level=2)
        subprocess.call(
            ["bench", "--site", args.site_name, "uninstall-app", app, "--yes"],
            cwd=os.getcwd() + "/" + args.bench_name,
        )

    # Install apps in the required order
